...
class LLMGenerator:
    ...
    def build_prompt(self, program_type: str, mood: str, lessons: str = "", creative: dict = None) -> str:
        ...
        prompt = (
            ...
            "- Start with variables, then main loop\n"       
            f"- Canvas: {canvas_w}x{canvas_h} pixels\n"
            "- RGB values are integers 0-255 (NOT floats 0.0-1.0)\n"
            f"{budget_rules}"
            "- There is NO input: no buttons, no keyboard, no mouse, no frame_count attribute.\n"
            "Programs must be fully autonomous and self-running. Track your own frame counter with a local variable if needed. Animate via time, math, or randomness.\n"            
            ...
            "  c.fill_triangle(x1,y1,x2,y2,x3,y3,r,g,b)\n"
            "  c.quad(x1,y1,x2,y2,x3,y3,x4,y4,r,g,b)\n"
            "  c.fill_quad(x1,y1,x2,y2,x3,y3,x4,y4,r,g,b)\n"
            "  c.translate(dx,dy)  # shift origin for following draws\n"
            "  c.rotate(theta)     # rotate following draws, theta in radians\n"
            "  c.push() / c.pop()  # save/restore the transform\n"
            "  c.show()\n"
            "  c.sleep(seconds)\n"
    ...
    def _build_simple_prompt(self, program_type: str, creative: dict = None) -> str:
        ...
        prompt = (
            ...
            f"- Canvas: {canvas_w}x{canvas_h} pixels\n\n"
            f"{budget_rules}\n"
            "Methods on 'c': clear, pixel, line, rect, fill_rect, circle, fill_circle, triangle, fill_triangle, quad, fill_quad, translate, rotate, push, pop, show, sleep\n"
            "All draw methods take r,g,b after coordinates. c.translate(dx,dy)/c.rotate(theta in radians)/c.push()/c.pop() transform following draws. c.show() takes no args.\n\n"
            f"{_python_output_rules(self.model_name)}"
        )
        return prompt
    ...
    def build_fix_prompt(self, code: str, error: str) -> str:
        ...
        prompt = (
            ...
            "- Keep it simple.\n"
            "- Use 'c' for drawing.\n"
            "- Check for commands that aren't allowed.\n\n"
            f"{_python_output_rules(self.model_name)}"
        )
        return prompt
